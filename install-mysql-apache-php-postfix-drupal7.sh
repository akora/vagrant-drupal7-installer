...
debconf-set-selections <<< "mysql-server mysql-server/root_password password root" # very, very bad practice, but hey, it's local dev ;)
...
then
  ...
  sed -i 's/127.0.0.1/0.0.0.0/g' $MySQL_config_file # very, very bad practice, don't do this on PROD
fi

mysql -uroot -proot -e "GRANT ALL PRIVILEGES ON *.* TO root@'%' IDENTIFIED BY 'root';" # very, very bad practice, but hey, it's local dev ;)
...
echo "=== Stopping and disabling Puppet and Chef agents..." # this is vagrant image specific; noticed them in syslog that they are running
...
drush dl backup_migrate ctools views auto_nodetitle token features field_group node_save_redirect
# drush dl demo logintoboggan nodeformcols
drush en color field_ui locale menu path taxonomy backup_migrate views views_ui token -y
